...
const PROFILE_ROWS: ProfileRow[] = [
  {
    ...
    fields: [{ key: 'username', label: 'New username', type: 'text', placeholder: 'e.g. john_doe' }],
  },
  {
    key: 'password',
    label: 'Password',
  ...
  },
  {
    ...
    displayValue: (user) =>
      user.preferred_topics
        ? user.preferred_topics.split(',').map((t) => t.trim()).join(', ')
        : '—',
    fields: [{ key: 'preferred_topics', label: 'Select topics', type: 'topics' }],
  },
  {
    key: 'language',
    label: 'Language',
...
export default function Profile() {
  ...
  async function handleAvatarUpload(file: File) {
    ...
    try {
      ...
      setAvatarError(extractError(err, 'Failed to upload avatar. Please try again.'));
    } finally {
  ...
  async function handleDeleteAvatar() {
    ...
    try {
      ...
      setAvatarError(extractError(err, 'Failed to remove avatar. Please try again.'));
    } finally {
      setAvatarLoading(false);
    }
  }
      ...
      <div className="flex flex-col md:flex-row gap-12 mt-4">
        ...
        <div className="flex-1 flex flex-col divide-y divide-fray-border border border-fray-border">
          ...
          <div className="flex items-center justify-between px-6 py-4">
            <div>
              <p className="text-xs font-semibold uppercase tracking-widest text-fray-text-faint mb-1">Email</p>
          ...
          {PROFILE_ROWS.map((row) => (
            <div key={row.key} className="flex items-center justify-between px-6 py-4">
              <div>
                <p className="text-xs font-semibold uppercase tracking-widest text-fray-text-faint mb-1">{row.label}</p>
                <p className="text-sm text-fray-text">{row.displayValue(user)}</p>
              </div>
              <button
